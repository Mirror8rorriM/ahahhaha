...
int CombatSystem::calculateIncomingDamage(int baseDamage, bool correctWord,
  ...
  if (effectiveTime <= 1.0)
    damageFactor = 0.0;
  else if (effectiveTime <= 2.0)
    damageFactor = 0.4;
  else if (effectiveTime <= 3.0)
    damageFactor = 0.7;
...
bool CombatSystem::fight(Hero& hero, EnemyData enemyTemplate) {
  ...
  std::cout << "Вводите одно из слов, написанных КАПСОМ. Чем быстрее ввод, тем "
               "меньше урон.\n\n";
  ...
  while (hero.isAlive() && enemyHp > 0) {
    ...
    if (correct && incoming == 0) {
      ...
      std::cout << "Вы успели среагировать частично. Получено урона: "
                << incoming << ".\n";
    } else {
      std::cout << "Неверное действие. Удар прошёл полностью. Получено урона: "
                << incoming << ".\n";
    }
